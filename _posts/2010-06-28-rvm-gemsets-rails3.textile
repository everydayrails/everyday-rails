...
The real work begins with the post-installation steps (scroll down about halfway on the "installation documentation":http://rvm.beginrescueend.com/rvm/install/). Locate your profile (by default on a Mac, that's <code>.bash_profile</code> inside your home directory; you may also have settings like <code>$PATH</code> in this file) and paste the following line at the bottom:
...
<div class="box">
  <strong>Which Rubies should I install?</strong> That depends on your particular development needs. I have 1.8.7, 1.9.1, and REE (Ruby Enterprise Edition) installed.
</div>
...
where <code>rails3</code> is your gemset's name. You can use another version of Ruby if you'd prefer&mdash;in fact, it might not be a bad idea to start getting familiar with Ruby 1.9.1 as long as you're enabling your computer to handle multiple Rubies and Rails versions.
...
<div class="box">
  Did you copy and paste the above and it didn't work? Remember, RVM installs Rubies and gems inside _your home folder_, so these settings point to those files. Unless your account name is also <code>everydayrails</code> you'll need to update these paths accordingly.
</div>
...
* Aside from integrating Passenger with your web server, _you won't use <code>sudo</code> to install gems anymore_.
