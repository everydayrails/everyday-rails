...
The "RVM installation documentation":http://rvm.beginrescueend.com/rvm/install/ lists three options. I used the first option (installing from the Github repository via a single command). I'm not sure why installing from the gem is not recommended, other than you're apt to not get the latest version.

The real work begins with the post-installation steps (scroll down about halfway on the "installation documentation":http://rvm.beginrescueend.com/rvm/install/). Locate your profile (by default on a Mac, that's <code>.bash_profile</code> inside your home directory) and paste the following line at the bottom:

{% highlight bash %}
  [[ -s "$HOME/.rvm/scripts/rvm" ]] && source "$HOME/.rvm/scripts/rvm"
{% endhighlight %}

You can type the <code>source</code> command shown in the instructions, or open a new shell, to begin using RVM.
...
At this point you can install Rails 2.3.8 in your default environment, and continue using it for your day-to-day development. You can also create a compartmentalized area for experimenting with Rails 3, called a _gemset_.
...
where 1.8.7 is the RVM version you're using, and rails3 is your gemset's name. You can use another version of Ruby if you'd prefer--in fact, it might not be a bad idea to start getting familiar with Ruby 1.9 as long as you're enabling your computer to handle multiple Rubies and Rails versions.
...
Finally, edit your web server configuration file (in the case of Apache on OS X, that's <code>/etc/apache2/httpd.conf</code>). This is what I wound up having to do to get mine to work:

{% highlight apache %}
  LoadModule passenger_module /Users/asumner/.rvm/gems/ruby-1.8.7-p249/gems/passenger-2.2.11/ext/apache2/mod_passenger.so
  PassengerRoot /Users/asumner/.rvm/gems/ruby-1.8.7-p249/gems/passenger-2.2.11
...
Then, follow the instructions provided in the "RVM documentation":http://rvm.beginrescueend.com/integration/databases/ to finish. Specifically, you'll add the following to <code>~/.rvmrc</code>:
...
(Adjust your path to MySQL's <code>bin</code> directory if needed; what's above is the default installation.)
...
<div class="box">
  There's a good Railscast episode about "using RVM with Rails 3 Beta":http://railscasts.com/episodes/200-rails-3-beta-and-rvm from last February that's also worth checking out.
</div>