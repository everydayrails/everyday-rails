...
Here's how it works. Most of the code resides in the <code>application_helper.rb</code> file:
...
You'll need to include <code>Rails.application.routes.url_helpers</code> in order to access <code>link_to</code> from a helper method. Then you add the helper method itself, which does nothing more than return a cancel link. Mine uses an old-style <code>:confirm</code> message; you can spruce it up with some less obtrusive if you'd like.
